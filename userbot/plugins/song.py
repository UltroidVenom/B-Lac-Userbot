...
async def nope(Jatt):
    Blac = Jatt.pattern_match.group(1)
    if not Blac:
        if Jatt.is_reply:
            (await Jatt.get_reply_message()).message
        else:
            await Jatt.edit(
    ...
    troll = await bot.inline_query("iLyricsBot", f"{(deEmojify(Blac))}")

    await troll[0].click(
        Jatt.chat_id,
        reply_to=Jatt.reply_to_msg_id,
        silent=True if Jatt.is_reply else False,
        hide_via=True,
    )
    ...
    async with borg.conversation("@DeezLoadBot") as conv:
        try:
            ...
            await borg.send_file(
                event.chat_id,
                songh,
                caption="🔆**Here's the requested song!**🔆\n`Check out` [BlacBot](https://t.me/BLAC_USERBOT)",
            )
            await event.delete()
        except YouBlockedUserError:
