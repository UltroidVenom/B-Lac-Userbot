...
# Now will be used in blacBot too....
...
async def set_not_afk(event):
    ...
    if ".afk" not in current_message and "yes" in USER_AFK:  # pylint:disable=E0602
        blacbot = await borg.send_message(
            event.chat_id,
            "🔥__Back alive!__\n**No Longer afk.**\n⏱️ `Was afk for:``"
            + total_afk_time
            + "`", file=blacpic
        ...
        try:
            ...
            await borg.send_message(  # pylint:disable=E0602
                ...
                + "Ask in @BLAC_USERBOT_GROUP to get help setting this value\n\n `{}`".format(str(e)),
                reply_to=event.message.id,
                silent=True,
            )
        await asyncio.sleep(5)
        await blacbot.delete()
...
async def on_afk(event):
  ...
  if reason
        ...
        msg = await event.reply(message_to_reply, file=blacpic)
...
async def _(event):
    if event.fwd_from:
        return
    jattop = await event.get_reply_message()
    global USER_AFK  # pylint:disable=E0602
    global afk_time  # pylint:disable=E0602
    global last_afk_message  # pylint:disable=E0602
    global afk_start
    ...
    blacpic = await event.client.download_media(jattop)
    if not USER_AFK:  # pylint:disable=E0602
        ...
        USER_AFK = f"yes: {reason} {blacpic}"  # pylint:disable=E0602
        if reason:
            await borg.send_message(
                event.chat_id, f"__**I'm going afk🚶**__ \n⚜️ Because `{reason}`", file=blacpic
            )
        else:
            await borg.send_message(event.chat_id, f"**I am Going afk!**🚶", file=blacpic)
        await asyncio.sleep(0.001)
        await event.delete()
        try:
            await borg.send_message(  # pylint:disable=E0602
                Config.PRIVATE_GROUP_BOT_API_ID,  # pylint:disable=E0602
                f"#AFKTRUE \nSet AFK mode to True, and Reason is {reason}",file=blacpic
            )
        except Exception as e:  # pylint:disable=C0103,W0703
